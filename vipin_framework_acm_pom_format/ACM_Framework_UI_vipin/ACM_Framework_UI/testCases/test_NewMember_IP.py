...
class Test_002_Creating_Member:
    ...
    # def test_Enrollment(self, setup):
        ...
        time.sleep(2)
        request_closed_text = self.driver.find_element(By.XPATH, "//*[@id='cmvCenterForm:cmvItemsTabView:1:filteredTaskAccordianPanel:closedTaskListTableFiltered_data']/tr[1]/td[3]").text
        request_outcome_text = self.driver.find_element(By.XPATH, "//*[@id='cmvCenterForm:cmvItemsTabView:1:filteredTaskAccordianPanel:closedTaskListTableFiltered_data']/tr[2]/td[3]")
        try:
            assert request_closed_text == request_outcome_text
            self.logger.info("*******ACM task and request Discharge successfully - Test Passed*******")
            print("Request and Task discharge text matched :" + request_closed_text)
        except:
            self.logger.error("******ACM task and request Discharge fail- Test Failed*******")
        task_closed = self.driver.find_element(By.XPATH,
                                       "(//*[@class='ui-accordion-header ui-helper-reset ui-state-default ui-state-active ui-corner-top'])[6]").text
        print(task_closed)
